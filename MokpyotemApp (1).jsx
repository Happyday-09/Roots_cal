        ...
        <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {[
              ...
              <button key={label} onClick={() => navigate(dest)} style={{
                background: "none", border: "none", cursor: "pointer",
                fontSize: 14, color: page === dest ? G.main : "#374151",
                fontWeight: page === dest ? 700 : 400,
                padding: "8px 12px",
                borderBottom: page === dest ? `2px solid ${G.main}` : "2px solid transparent",
              }}>{label}</button>
            ))}
          </div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <Btn onClick={() => navigate("login")} variant="outline" style={{ padding: "8px 20px", fontSize: 13 }}>로그인</Btn>
          <Btn onClick={() => navigate("signup")} style={{ padding: "8px 20px", fontSize: 13 }}>회원가입</Btn>
        </div>
      </div>
    </nav>
  );
    ...
    <div style={{ marginBottom: 18 }}>
      ...
      <input
        ...
        style={{
          ...
          border: `1.5px solid ${error ? "#F87171" : focused ? G.main : "#E5E7EB"}`,
...
// ─── Social Button ────────────────────────────────────────────────
function SocialBtn({ emoji, label, color, bg }) {
  return (
    <button style={{
      width: "100%", padding: "12px", borderRadius: 10,
      border: "1.5px solid #E5E7EB", background: bg || "#fff",
      display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
      fontSize: 14, fontWeight: 600, color: color || "#374151", cursor: "pointer",
      marginBottom: 10,
    }}
      onMouseEnter={e => e.currentTarget.style.opacity = "0.85"}
      onMouseLeave={e => e.currentTarget.style.opacity = "1"}
    ...
    <div style={{ minHeight: "calc(100vh - 64px)", background: "#F3F4F6", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 24px" }}>
      <div style={{ width: "100%", maxWidth: 440 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ width: 52, height: 52, background: G.main, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, margin: "0 auto 14px" }}>📈</div>
        ...
        <Card style={{ padding: "36px 32px" }}>
          {/* Social Login */}
          <SocialBtn emoji="🟡" label="카카오로 로그인" color="#3A1D1D" bg="#FEE500" />
          <SocialBtn emoji="🟢" label="네이버로 로그인" color="#fff" bg="#03C75A" />
          <SocialBtn emoji="🔵" label="구글로 로그인" />
          ...
          <Field label="이메일" type="email" placeholder="example@email.com" value={email} onChange={setEmail} error={errors.email} />
          <Field label="비밀번호" type="password" placeholder="비밀번호 입력" value={pw} onChange={setPw} error={errors.pw} />

          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 24, marginTop: -8 }}>
            <button style={{ background: "none", border: "none", fontSize: 12, color: G.dark, cursor: "pointer", fontWeight: 500 }}>비밀번호를 잊으셨나요?</button>
          </div>

          <Btn onClick={handleLogin} style={{ width: "100%", padding: "14px", fontSize: 15 }}>로그인</Btn>

          <p style={{ textAlign: "center", fontSize: 13, color: "#6B7280", marginTop: 20 }}>
            아직 회원이 아니신가요?{" "}
            <button onClick={() => navigate("signup")} style={{ background: "none", border: "none", color: G.main, fontWeight: 700, cursor: "pointer", fontSize: 13 }}>
              회원가입
            </button>
          </p>
        </Card>
...
function SignupPage({ navigate }) {
  ...
  const pwStrength = () => {
    const p = form.pw;
    if (!p) return null;
    if (p.length < 6) return { label: "약함", color: "#EF4444", width: "33%" };
    if (p.length < 10 || !/[^a-zA-Z0-9]/.test(p)) return { label: "보통", color: "#F59E0B", width: "66%" };
    return { label: "강함", color: G.main, width: "100%" };
  };
    ...
    return (
      <div style={{ minHeight: "calc(100vh - 64px)", background: "#F3F4F6", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 24px" }}>
        <div style={{ textAlign: "center", maxWidth: 400 }}>
          <div style={{ width: 88, height: 88, background: G.light, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 44, margin: "0 auto 24px" }}>🎉</div>
          <h2 style={{ fontSize: 26, fontWeight: 800, color: "#111827", margin: "0 0 12px" }}>
            {form.name}님, 환영해요!
          </h2>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <Btn onClick={() => navigate("goal")} style={{ width: "100%", padding: "14px", fontSize: 15 }}>첫 목표 설정하기 →</Btn>
            <Btn onClick={() => navigate("login")} variant="outline" style={{ width: "100%", padding: "14px", fontSize: 14 }}>로그인 페이지로</Btn>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 64px)", background: "#F3F4F6", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 24px" }}>
      <div style={{ width: "100%", maxWidth: 480 }}>
        ...
        <Card style={{ padding: "36px 32px" }}>
          {/* Social Signup */}
          <SocialBtn emoji="🟡" label="카카오로 시작하기" color="#3A1D1D" bg="#FEE500" />
          <SocialBtn emoji="🟢" label="네이버로 시작하기" color="#fff" bg="#03C75A" />
          ...
          <Field label="이름" placeholder="홍길동" value={form.name} onChange={(v) => set("name", v)} error={errors.name} />
          <Field label="이메일" type="email" placeholder="example@email.com" value={form.email} onChange={(v) => set("email", v)} error={errors.email} />
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <input
              ...
              style={{ width: "100%", padding: "12px 16px", border: `1.5px solid ${errors.pw ? "#F87171" : form.pw && strength ? strength.color : "#E5E7EB"}`, borderRadius: 10, fontSize: 14, outline: "none", boxSizing: "border-box", color: "#111827" }}
            />
            {form.pw && strength && (
              <div style={{ marginTop: 8 }}>
                ...
                <p style={{ fontSize: 11, color: strength.color, margin: "4px 0 0", fontWeight: 600 }}>비밀번호 강도: {strength.label}</p>
              </div>
            )}
            {errors.pw && <p style={{ fontSize: 12, color: "#EF4444", margin: "5px 0 0" }}>{errors.pw}</p>}
          </div>

          <Field label="비밀번호 확인" type="password" placeholder="비밀번호 재입력" value={form.pwConfirm} onChange={(v) => set("pwConfirm", v)} error={errors.pwConfirm} />
          ...
          <div style={{ marginTop: 4, marginBottom: 24 }}>
            <div style={{
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", marginBottom: 12 }}>
                <input
                  type="checkbox"
                  checked={form.agree && form.agreeMarketing}
                  onChange={(e) => { set("agree", e.target.checked); set("agreeMarketing", e.target.checked); }}
                  style={{ width: 16, height: 16, accentColor: G.main }}
                />
              ...
              <div style={{ borderTop: "1px solid #E5E7EB", paddingTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
                {[
                  { key: "agree", label: "서비스 이용약관 동의", required: true },
                  { key: "agreeMarketing", label: "마케팅 정보 수신 동의", required: false },
                ].map(({ key, label, required }) => (
                  <label key={key} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                    <input type="checkbox" checked={form[key]} onChange={(e) => set(key, e.target.checked)} style={{ width: 15, height: 15, accentColor: G.main }} />
                    <span style={{ fontSize: 13, color: "#374151" }}>
                      {required && <span style={{ color: G.main, marginRight: 4 }}>[필수]</span>}
                      {!required && <span style={{ color: "#9CA3AF", marginRight: 4 }}>[선택]</span>}
                      {label}
                    </span>
                  </label>
                ))}
            ...
            {errors.agree && <p style={{ fontSize: 12, color: "#EF4444", margin: "6px 0 0" }}>{errors.agree}</p>}
          </div>

          <Btn onClick={handleSubmit} style={{ width: "100%", padding: "14px", fontSize: 15 }}>회원가입</Btn>

          <p style={{ textAlign: "center", fontSize: 13, color: "#6B7280", marginTop: 20 }}>
            이미 계정이 있으신가요?{" "}
            <button onClick={() => navigate("login")} style={{ background: "none", border: "none", color: G.main, fontWeight: 700, cursor: "pointer", fontSize: 13 }}>
              로그인
            </button>
          </p>
        </Card>
